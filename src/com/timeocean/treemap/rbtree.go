...
// do subtree
func (t *RbTree) DoSubTree(fromK Key, toK Key, f func(k Key, v interface{})) {
        inOrderDoSubTree(t.root, fromK, toK, f);
}

func inOrderDoSubTree(n * Node, fromK Key, toK Key, f func(k Key, v interface{})){
		if n != &nilNode {
			if fromK.Compare(n.Key) < 0 {
				inOrderDoSubTree(n.left, fromK, toK, f);
			}
			if fromK.Compare(n.Key) <= 0 && toK.Compare(n.Key) >= 0{
				f(n.Key, n.Value);
			}
			if toK.Compare(n.Key) > 0 {
				inOrderDoSubTree(n.right, fromK, toK, f);
			}
		}	
}
